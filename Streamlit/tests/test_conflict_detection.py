...
class TestConflictDetection(unittest.TestCase):
    ...
    def test_visualization_smoke(self):
        """Sanity check that the visualization helpers execute.

        This test does not verify the graphical output; it simply ensures
        that calling the two public functions does not raise an exception in a
        headless test environment.  A minimal layout and a single vehicle are
        enough to exercise the drawing/animation logic.
        """

        from src.visualization import visualize_intersection, animate_vehicles

        layout = {'lanes': [{'direction': 'north', 'width': 3, 'length': 100}]}

        # first exercise with a plain dict (legacy use case)
        vehicles_dict = [
            {
                'vehicle_id': 'X1',
                'lane': 1,
        ...
        # functions now return the Plotly figure so we can inspect annotations
        fig = visualize_intersection(layout, vehicles_dict, steps=2, interval=10)
        self.assertIsNotNone(fig)
        annos = getattr(fig.layout, 'annotations', []) or []
        texts = [a.text for a in annos]
        for compass_label in ['N', 'S', 'E', 'W']:
            self.assertIn(compass_label, texts)
        # alias still works (return value ignored)
        animate_vehicles(layout, vehicles_dict, steps=2, interval=10)

        # now exercise with Vehicle objects produced by parse_vehicles
        scenario = {'vehicles_scenario': vehicles_dict}
        vehicles_objs = parse_vehicles(scenario, {'north': {'1': ['H']}})
        visualize_intersection(layout, vehicles_objs, steps=2, interval=10)
        animate_vehicles(layout, vehicles_objs, steps=2, interval=10)
